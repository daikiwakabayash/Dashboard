...
export const ENDPOINTS = {
  ...
  // 新規客の一覧（1人1行・CPA計測用）。received_at(受付日時)/ltv/joined/visited_by_time 等。ページング(limit/cursor)。
  'marketing/new-customers': { path: '/marketing/new-customers', params: ['from', 'to', 'shop_id', 'limit', 'cursor'], required: ['from', 'to'], kind: 'summary' },
  // 強制リンク（予約フォーム）別の実績。data.rows/totals/unattributed。初回オファー/訴求（肩こり・腰痛等）の効果集計用。
