...
describe('ローカル結合（実ハンドラ+擬似KV）: @AI 実接続', () => {
  ...
  //    ①が修正済み（受付の版照合＋原子的追記）。通常の回帰テストとして常に通ること。
  it('🔴 【修正済み】同じ request_id の同時送信でも回答は1つだけであるべき', async () => {
...
describe('ローカル結合（実ハンドラ+擬似KV+擬似上流）: 回答生成中の同時操作', () => {
  ...
  //    ①が修正済み（差分だけを版照合して書く）。通常の回帰テストとして常に通ること。
  it('🔴 【修正済み】生成中に行った本部の訂正が、生成完了時の保存で消えない', async () => {
  ...
  it('🔴 【修正済み】別々の送信を同時に出しても、どちらの回答も失われない', async () => {
  ...
  it('🔴 【修正済み】生成中にフラグを OFF にしたら、その回答は投稿されない', async () => {
    kv.upstream.delayMs = 250;
