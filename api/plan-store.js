...
export default async function handler(req, res) {
  ...
  if (ccType === 'meta') {
    ...
    const tenantId = String(req.query.tenantId || 'naoru').slice(0, 64);
    const period = (req.query.from && req.query.to)
      ? { from: String(req.query.from).slice(0, 10), to: String(req.query.to).slice(0, 10) }
      : lastCompleteDays(7);
    const accountId = String(req.query.accountId || process.env.META_AD_ACCOUNT_ID || '').slice(0, 64);
    ...
    try {
      ...
      const data = normalizeMetaOverview(raw, { fixture: false });
      return res.status(200).json({
        ok: true,
