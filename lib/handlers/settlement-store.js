...
import { hashOwnerToken, parseOwnerPasswords, verifyOwnerToken, normalizeRecord, parseOwnerShops, allowedShopsFor, SETTLEMENT_STATUS } from '../settlement.js';
import { verifySalonOneBearer, bearerFromReq } from '../salonone-auth.js';

// サロンワンSSOの store コンテキスト（レコードが許可店舗か判定）
function soRecordAllowed(so, r) {
  if (!so) return false;
  if (so.root) return true;
  const rid = String(r.shopId || '');
  if (rid && so.shopIds.includes(rid)) return true;
  const nm = String(r.shopName || '');
  return so.shopNames.some(p => p && nm.includes(p));
}
...
export default async function handler(req, res) {
  ...
  // サロンワンSSOのトークン（Bearer）があれば検証。brand_admin=本社相当、それ以外=アクセス店舗に絞ったオーナー相当。
  const bearer = bearerFromReq(req);
  const so = bearer ? await verifySalonOneBearer(bearer) : null;
  ...
  if (req.method === 'GET') {
    ...
    const root = isRoot(owner, token) || (hqToken && hqTokenValid(hqToken)) || (so && so.root);
    try {
      const { passwords, shopsMap } = await loadAccounts(gasUrl);
      // 認可: root/本社 は全件、サロンワンSSOはアクセス店舗、オーナーは本人トークン必須
      if (!root && !so) {
      ...
      const allowed = (!root && !so && owner) ? allowedShopsFor(shopsMap, owner) : null;
      const records = rows.map(normalizeRecord).filter(Boolean).filter(r => {
        if (root) return true;
        if (so) return soRecordAllowed(so, r);        // サロンワンSSO: アクセス店舗のみ
  ...
  try {
    ...
    if (action === 'publish') {
      if (!hqTokenValid(body.hqToken) && !(so && so.root)) return res.status(401).json({ ok: false, error: '本社認証が必要です' });
    ...
    if (action === 'confirm' || action === 'requestRevision') {
      const root = isRoot(body.owner, body.token) || (so && so.root);
      const shopId = String(body.shopId || '');
      const month = String(body.month || '');
      if (!shopId || !/^\d{4}-\d{2}$/.test(month)) return res.status(400).json({ ok: false, error: 'shopId と month が必要です' });
      // 認可: root=全て可 / サロンワンSSO=アクセス店舗のみ / 従来オーナー=本人トークン
      let byOwner = '';
      if (!root) {
        if (so) {
          if (!so.shopIds.includes(shopId)) return res.status(403).json({ ok: false, error: 'この店舗の操作権限がありません' });
          // 店舗単位で認可済み。GASは shopId+month で一意なので owner 絞りは不要。
        } else {
      ...
      // GAS 側は owner 一致行のみ更新（root/サロンワンSSOは owner を渡さず shopId+month で更新）
      const out = await callGas(gasUrl, 'POST', {
        action: 'update', owner: byOwner, shopId, month, updatedAt: now, ...patch,
      });
