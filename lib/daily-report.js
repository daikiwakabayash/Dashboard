...
// ── ② プレオープン期間（preopen）─────────────────────────────────────────
// 施術が始まっているので当日の実績が出る。同時に本オープンまでの集客も見たい。
// ⚠️ この期間の項目立てはオーナーの確認待ち。いまは「当日の実績＋本オープンまでの集客」を出す。
export function buildPreOpenReport(input) {
  ...
  const chs = Array.isArray(i.channels) ? i.channels : null;
  const missing = [];
  if (sum == null) missing.push('当日の実績');
  if (chs == null) missing.push('媒体別の予約');
  ...
  const curSnap = chs ? snapshotOf(chs) : {};
  const delta = dailyDelta(i.prevSnap || null, curSnap);
  const booked = chs ? sumStrict(delta.rows.map(r => r.total)) : null;
  const target = s.targetNew;
  const remain = (target != null && isNum(booked)) ? target - booked : null;

  const card = {
    kind: PREOPEN,
    brand: 'NAORU × SalonOne',
    title: PHASE_LABEL.preopen,
    badge: 'プレオープン',
    atLabel: `${ymdLabelFull(ymd) || ymd} ${fetchedAt != null ? jstHm(fetchedAt) : ''}時点・日本時間`.replace('  ', ' '),
    milestones: [{ label: '本オープンまで', value: ymdLabel(s.openDate) || '未設定' }],
    today: [
      { label: '本日の売上', value: gross, unit: '円', money: true },
      { label: '本日の来店', value: visits, unit: '名' },
      { label: '本日の入会', value: joins, unit: '名' },
    ],
    goal: {
      headline: target == null ? '目標が未設定です'
        : remain == null ? `本オープンまで、あと${NOT_FETCHED}`
        : remain > 0 ? `本オープンまで、あと${people(remain)}` : `目標を達成（${people(booked)}）`,
      note: target == null ? '設定画面で目標人数を入れてください' : `目標${people(target)}`,
      current: booked, target, rate: rate(booked, target),
    },
    channels: { header: '媒体別の予約状況', note: '本オープン月ぶんの累計', rows: delta.rows,
                total: { added: delta.today.added, cancelled: delta.today.cancelled, total: booked } },
    footer: { source: `SalonOne・更新 ${ymdLabel(ymd)} ${fetchedAt != null ? jstHm(fetchedAt) : ''}`.trim(),
              schedule: `毎日${s.sendAt}配信` },
  };

  const lines = [];
  lines.push(`【プレオープン 日次レポート】${shopName}`);
  lines.push(card.atLabel);
  lines.push('');
  lines.push('■ 本日の実績');
  lines.push(`　売上 ${yen(gross)}`);
  lines.push(`　来店 ${people(visits)}（新規 ${people(newVisit)} ／ 既存 ${people(repeatVisit)}）`);
  lines.push(`　入会 ${people(joins)}${joinRate == null ? '' : `（入会率 ${percent(joinRate)}）`}　キャンセル ${people(cancel, '件')}`);
  lines.push('');
  lines.push(`■ 本オープンに向けた集客（本オープン ${ymdLabel(s.openDate) || '未設定'}）`);
  lines.push(`　${card.goal.headline}`);
  lines.push(`　累計の有効予約人数 ${people(booked)} ／ ${card.goal.note}`);
  if (delta.rows.length) {
    for (const r of delta.rows.slice(0, 10)) {
      lines.push(`　${r.name}　新規 ${people(r.added, '件')} ／ 累計 ${people(r.total)}`);
    }
  }
  lines.push('');
  lines.push(...footer({
    sources: ['sales/summary', 'marketing/by-channel'],
    period: `${ymd}（当日の実績）／ 本オープン月（集客の累計）`,
    fetchedAt,
    note: '「集計実行」の前に取得した場合、当日の売上は確定前の値になることがあります。',
  }));
  ...
  return {
    ...
    text: lines.join('\n'), lines, card, missing,
    snapshot: curSnap,
    facts: { gross, visits, newVisit, repeatVisit, joins, joinRate, cancel, booked, target, remain },
  };
}
...
export function planRun({ settings, log, nowMs }) {
  ...
  for (const s of targetsFor(settings)) {
    ...
    // prep / preopen は指定時刻に出す（「集計実行」が無い期間）
