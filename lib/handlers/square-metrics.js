...
async function fetchAccountData(tokenConfig, isMultiAccount) {
  ...
  try {
    ...
    const SUBS_TIMEOUT = 40000; // サブスクは未来予定決済の唯一の源→予算を拡大し途中打ち切りを防止（並列なので全体時間は不変）
...
// 会員数・MRR・総売上の正確性に直結するAPI（invoices/payments/refunds）が
// タイムアウト＝部分結果のときだけキャッシュを抑止する。
// subscriptions は副次情報（解約内訳・新規開始日）で、部分でも上記の数値には影響しないため
// 部分でもキャッシュ可（次回SYNCを即時化しつつ、正確な数値は完全なまま保つ）。
function isCriticalPartial(r) {
  if (!r || !r._diag) return false;
  return ['invoicesApi', 'paymentsApi', 'refundsApi'].some(k => {
    const v = r._diag[k];
