...
import { parseOwnerPasswords, verifyOwnerLogin, verifyOwnerToken } from '../lib/settlement.js';
...
export default function handler(req, res) {
  ...
  // オーナー名の一覧のみ返す（ログイン画面のプルダウン用・PASSは返さない）
  if (req.method === 'GET') {
    res.setHeader('Cache-Control', 'no-store');
    return res.status(200).json({ configured, owners: Object.keys(passwords) });
  }
  ...
  if (action === 'login') {
    ...
    res.setHeader('Cache-Control', 'no-store');
    return res.status(200).json({ ok: true, owner: r.owner, token: r.token });
  }
