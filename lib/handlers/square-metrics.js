...
async function fetchAccountData(tokenConfig, isMultiAccount) {
  ...
  try {
    ...
    // ⚠️ subscriptions は「未来月の会員数・予定決済（chargedThroughDate）」の唯一のデータ源。
    //    以前は副次情報扱いで24秒に絞っていたが、これが原因で大型店（千葉/大森等 約半数）の
    //    サブスク取得が途中打ち切りになり、未来月の人数が過少になっていた。
    //    4本は Promise.all で並列＝アカウント全体の所要時間は「最も遅いAPI(invoices/payments=48秒)」で決まるため、
    //    subscriptions の予算を40秒に上げても全体の所要時間は増えない（invoicesの48秒に内包）。
    //    完走率を上げて未来月の取りこぼしを防ぐ。plan詳細(サブスク後にチェーン)分の余裕として48秒未満に留める。
    const SUBS_TIMEOUT = 52000; // サブスクは未来予定決済の唯一の源→予算を最大化し途中打ち切りを防止（過去分カットでinvoices/paymentsが早く終わるので余裕あり・60秒キル手前）
