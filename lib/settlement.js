...
// shokeiTotal = セラピスト諸経費（住宅/美容手当等）の合計。請求額から差し引く（値引き）。
export function computeSettlement({ cash = 0, squareSales = 0, hpb = 0, royaltyRate = 15, taxRate = 10,
                                    squareFees = 0, spotip = 0, metaTotal = 0, adjTotal = 0, shokeiTotal = 0 }) {
  const royaltyBase = NUM(cash) + NUM(squareSales) + NUM(hpb);
  const royalty = Math.round(royaltyBase * NUM(royaltyRate) / 100);
  const billTotal = royalty + NUM(squareFees) + NUM(spotip) + NUM(metaTotal) + NUM(adjTotal) - NUM(shokeiTotal);
...
// ── セラピスト諸経費（手当）の自動判定 ─────────────────────────────
// 業務委託の偽装リスク回避のため、請求書上の名目は「諸経費（氏名）」に統一し、
// 内訳（住宅/美容/子供/誕生日/実費）は注釈で示す。
//
// 自動判定ルール（税込生産性 productivity を指標に）:
//   - 生産性 > 100万円 → 住宅手当2万 ＋ 美容手当1万 ＝ 計3万（自動計上）
//   - 生産性 ≤ 100万円 → 自動計上なし（0円。その他は明細提出で手入力）
// 子供1人5,000円・誕生日月1万円・その他実費(extras)は生産性に関わらず加算。
export const SHOKEI_RULES = {
  threshold: 1000000, // 税込生産性のしきい値（超で住宅/美容が自動計上）
  housing: 20000,     // 住宅手当（>100万 のとき）
  beauty: 10000,      // 美容手当（>100万 のとき）
  childUnit: 5000,    // 子供1人あたり
  birthdayUnit: 10000,// 誕生日月
};

// 1セラピストの諸経費を算出。t = { children, birthday, extras:[{label,amount}] }
export function computeTherapistShokei(t = {}, productivity = 0) {
  const over = NUM(productivity) > SHOKEI_RULES.threshold;
  const housing = over ? SHOKEI_RULES.housing : 0;
  const beauty = over ? SHOKEI_RULES.beauty : 0;
  const childrenCount = Math.max(0, Math.floor(NUM(t.children)));
  const childrenAmt = childrenCount * SHOKEI_RULES.childUnit;
  const birthdayAmt = t.birthday ? SHOKEI_RULES.birthdayUnit : 0;
  const extras = Array.isArray(t.extras) ? t.extras : [];
  const extrasAmt = extras.reduce((a, x) => a + NUM(x && x.amount), 0);
  const total = housing + beauty + childrenAmt + birthdayAmt + extrasAmt;
  const parts = [];
  if (housing) parts.push('住宅手当2万');
  if (beauty) parts.push('美容手当1万');
  if (childrenAmt) parts.push(`子供${childrenCount}名(¥${childrenAmt.toLocaleString()})`);
  if (birthdayAmt) parts.push('誕生日1万');
  extras.forEach(x => { const a = NUM(x && x.amount); if (a) parts.push(`${(x && x.label) || '実費'}(¥${a.toLocaleString()})`); });
  return { over, housing, beauty, childrenCount, childrenAmt, birthdayAmt, extrasAmt, total, note: parts.join('・') };
}
