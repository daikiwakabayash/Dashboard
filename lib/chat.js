...
// リアクションのトグル（1ユーザーにつき最大1絵文字）。
//   - 同じ絵文字を再度押す → 解除（オフ）
//   - 別の絵文字を押す → 付け替え（前の絵文字は自動で外れる）
// 1メッセージ/1ノートにつき同じ人は1種類のみ。reactions を新しいオブジェクトで返す。
export function toggleReaction(reactions, emoji, staffId) {
  const src = reactions && typeof reactions === 'object' ? reactions : {};
  const sid = String(staffId || '');
  const had = Array.isArray(src[emoji]) && src[emoji].map(String).includes(sid);
  const r = {};
  // まず全絵文字から自分を外す（1ユーザー1絵文字を保証）
  for (const key of Object.keys(src)) {
    const arr = (Array.isArray(src[key]) ? src[key] : []).map(String).filter(x => x !== sid);
    if (arr.length) r[key] = arr;
  }
  // 押した絵文字が「元々付いていなかった」場合のみ新規付与（付いていた＝解除で終了）
  if (!had) r[emoji] = [...(Array.isArray(r[emoji]) ? r[emoji] : []), sid];
  return r;
}
