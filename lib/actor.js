...
export async function resolveActor(req, deps) {
  ...
  // ⚠️ クエリ文字列からは**受けない**。URLはアクセスログ・Referer・履歴に残るため、
  //    トークンを載せる場所として適さない。GET でもヘッダを使う。
  // ⚠️ HTTPヘッダは ISO-8859-1 しか運べない。アカウント名は日本語のことがある
  //    （本部アカウントは個人名）ため、フロントは percent-encode して送る。
  ...
  if (owner && token && d.verifyOwnerToken && d.loadAccounts) {
    ...
    if (acct && d.verifyOwnerToken(acct.passwords, owner, token)) {
      ...
      return normalizeActor({
        ...
        // 本部アカウントに紐付けた staffId も本人として扱う（画面はこちらで投稿する）
        altIds: [meta.staffId].filter(v => v != null && v !== '').map(String),
        verified: true,
      });
    }
  }
  ...
  if (bearer && d.verifySalonOneBearer) {
    ...
    if (so) {
      const actor = normalizeActor({
        ...
        // ⚠️ 画面は staff_id を本人IDに使う。user_id と別番号のことがあるため、
        //    両方を本人の識別子として持つ（本人の下書き・DMが見えなくなるのを防ぐ）。
        altIds: [so.staffId].filter(v => v != null && v !== '').map(String),
        verified: true,
      });
