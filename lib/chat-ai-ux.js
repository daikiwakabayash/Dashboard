...
//   sources には「根拠として数えてよいもの」だけを渡すこと（未検証の候補は含めない）。
...
export function buildAiAnswerMessage(input = {}) {
  ...
  // 出典は「検証済み(verified)」と「渡しただけの候補(candidates)」を必ず分ける。
  //   - 旧形式（sources 配列だけ）で呼ばれた場合は **候補扱い**（安全側。勝手に検証済みにしない）
  const si = input.sourceInfo && typeof input.sourceInfo === 'object' ? input.sourceInfo : null;
  const norm = (list) => arr(list).map(s => ({
    kind: ['faq', 'knowledge', 'shop_data'].includes(str(s && s.kind)) ? str(s.kind) : 'faq',
    id: str(s && (s.docId ?? s.doc_id ?? s.id)).slice(0, 64),
    title: str(s && s.title).slice(0, 120),
    version: str(s && s.version).slice(0, 40),
    updatedAt: str(s && (s.updatedAt ?? s.updated_at)).slice(0, 40),
    locator: str(s && s.locator).slice(0, 80),
  })).slice(0, 5);

  const verified = norm(si ? si.verified : []);
  const candidates = norm(si ? si.candidates : input.sources);
  let verification = str(si && si.verification);
  if (!['server_verified', 'unverified', 'none'].includes(verification)) {
    verification = verified.length ? 'server_verified' : (candidates.length ? 'unverified' : 'none');
  }
  if (verification === 'server_verified' && !verified.length) verification = candidates.length ? 'unverified' : 'none';

  const confidence = num(input.confidence);
  // ⚠️ 根拠として数えるのは **検証済みの出典だけ**。候補があっても断定しない。
  const esc = shouldEscalate({
    needsHq, sources: verified, confidence,
  ...
  return {
    ...
    ai: {
      ...
      mode: str(input.mode) === 'live' ? 'live' : 'sample',
      sourceVerification: verification,
      sources: verified,          // 表示してよい「出典」
      candidates,                 // 「渡しただけ」の参照候補
      confidence,
      ...
      answerMessageId: str(input.answerMessageId) || '',
      questionMessageId: str(input.questionMessageId) || '',
    },
  };
}
...
export function buildAgentRunOutcome(message, extra = {}) {
  ...
  return {
    ...
    result: {
      ...
      level: str(ai.level),
      sourceVerification: str(ai.sourceVerification),
      sourceCount: arr(ai.sources).length,          // 検証済みの出典の件数
      candidateCount: arr(ai.candidates).length,     // 渡しただけの候補の件数
