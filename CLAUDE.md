...
- **マーケティングタブ**（サイドバー独立・id='mktg'・root専用）: SalonOne売上から分離。サブタブ=ダッシュボード(soView='acq')／強制リンク別(soView='forced')／新規顧客一覧(soView='customers')。SalonOne売上は 店舗/スタッフ/日報/計画 のみに整理。ページは currentPage∈{salonone,mktg} で同一コンテナを描画し、サブタブとセクションを currentPage/soView で出し分け（?tab=mktg 深リンク可）
