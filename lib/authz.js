...
export function normalizeActor(raw) {
  ...
  return {
    ...
    // 同一人物の別ID（SSOの staff_id / user_id、本部アカウントの紐付け staffId）
...
// ── ルームの可視判定（チャットのアクセス制御の正本）────────────────────
// ⚠️ 権限判断はこの1ファイルに集約する。チャット側に第二の権限システムを作らない。
//   room = { id, kind:'announce'|'store'|'group'|'dm', shop, members:[staffId] }

// 同一人物の別ID（SSOの staff_id / user_id、本部アカウントに紐付けた staffId）も本人として扱う。
// これが無いと、本人なのに自分のDMが見えないという取り違えが起きる。
export function actorIdSet(actor) {
  const a = normalizeActor(actor);
  return [a.id, ...(Array.isArray(a.altIds) ? a.altIds : [])].map(String).filter(Boolean);
}
...
export function isRoomMember(actor, room) {
  ...
  return actorIdSet(actor).some(id => mem.includes(id));
}
...
export function canViewRoom(actor, room) {
  ...
  if (!r) return false;
  if (!a.verified) return false;                      // 本人確認できていない主体には出さない
  // テナントが違えば役割に関わらず見えない
  if (r.tenantId && normTenant(r.tenantId) !== a.tenantId) return false;
  const kind = String(r.kind || '');
  // DM は当事者だけ。**root / 本部でも非参加のDMは見えない**（ここを開けると誰も使わなくなる）
  if (kind === 'dm') return isRoomMember(a, r);
  // グループはメンバーのみ。root / 本部は運営上の必要から見える
