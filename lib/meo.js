...
export function computeDeltas(history, now = new Date()) {
  ...
  // 今月最初のスナップショット（＝今月頭の基準）。無ければ先月以前の最後を基準に。
  const firstThisMonth = list.find(h => String(h.date).slice(0, 7) === ym);
  const baseForMonth = firstThisMonth || prev || last;
  // 約30日前に最も近いスナップショット
  ...
  return {
    ...
    newThisMonth: firstThisMonth ? last.count - firstThisMonth.count : (prev ? last.count - prev.count : null), // 今月の新規口コミ
    delta30: s30 ? last.count - s30.count : null,                     // 約30日前比
    hasHistory: list.length >= 2,
    baseMonthDate: baseForMonth ? baseForMonth.date : '',
  };
}
