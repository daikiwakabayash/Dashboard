...
// ⚠️ **①に既定値を持たない。** 上限は③のテンプレの都合で変わるので、①が数値を
// 焼き込むと、③がテンプレを変えた瞬間に画面だけ古い上限を出す。取れなければ
// 「未取得」のまま扱い、**送信自体を止める**（422になるまで試さない）。
export const TEXT_FIELDS = Object.freeze(['headline', 'body', 'cta']);
export const TEXT_FIELD_LABEL = Object.freeze({ headline: '見出し', body: '本文', cta: 'ボタンの文言' });

export function normalizeLimits(raw) {
  const s = (raw && typeof raw === 'object') ? raw : {};
  const t = (s.text_limits && typeof s.text_limits === 'object') ? s.text_limits : {};
  const num = (v) => (typeof v === 'number' && Number.isFinite(v) && v > 0 && v <= 1e9) ? Math.floor(v) : null;
  const text = {};
  for (const k of TEXT_FIELDS) { const n = num(t[k]); if (n !== null) text[k] = n; }
  if (!TEXT_FIELDS.every(k => text[k] !== undefined)) {
    return { loaded: false, reason: '③が文字数上限を返しませんでした', text: {},
             revisionMax: null, maxBytes: null, apiVersion: '', templateVersion: '' };
  }
  return { loaded: true, reason: '', text,
           revisionMax: num(s.revision_instructions_max), maxBytes: num(s.max_artifact_bytes),
           apiVersion: str(s.api_version, 64), templateVersion: str(s.template_version, 64) };
}

// 上限に対する超過。⚠️ 上限が未取得なら known:false（「超過なし」ではない）。
export function textOverflows(limits, values = {}) {
  if (!limits || !limits.loaded) return { known: false, errors: [] };
  const errors = [];
  for (const k of TEXT_FIELDS) {
    const v = values[k];
    if (v === undefined || v === null) continue;
    const got = String(v).length, max = limits.text[k];
    if (got > max) errors.push({ field: k, max, got });
  }
  return { known: true, errors };
}

// ③へ送ってよいか。**上限が未取得なら送らない。**
export function textSendable(limits, values = {}) {
  const r = textOverflows(limits, values);
  if (!r.known) return { ok: false, error: 'limits_unavailable', errors: [] };
  if (r.errors.length) return { ok: false, error: 'text_too_long', errors: r.errors };
  return { ok: true, error: '', errors: [] };
}

// ③の 422 error.field/max/got を、人が読める1行にする。
export function textErrorMessage(errors) {
  const list = (Array.isArray(errors) ? errors : []).filter(e => e && e.field);
  if (!list.length) return '';
  return list.map(e => `${TEXT_FIELD_LABEL[e.field] || e.field}が${e.max}文字を超えています（${e.got}文字）`).join(' / ');
}
...
export function buildGenerateRequest(creative, asset, opts = {}) {
  ...
  if (rev) {
    ...
    // ③は version>1 で「明示の文言変更が1つも無い」依頼を 422
    // STRUCTURED_TEXT_CHANGE_REQUIRED で拒否する。**送る前にここで止める。**
    // （自由文だけで「直した」ことにしない、という③の契約に①側から合わせる）
    if (!TEXT_FIELDS.some(k => String(req.text_changes[k] || '').trim())) {
      return { ok: false, error: 'text_changes_required' };
    }
...
export function requestRevision(creative, input = {}, ctx = {}, t) {
  ...
  const pick = (k, n) => (explicit[k] !== undefined ? explicit[k] : str(creative[k], n));
  // ⚠️ 空文字は入れない。③は空の文言変更を422で拒否するので、送っても必ず失敗する。
  // 「消す」は③のテンプレに表現が無いため、①からは送らない（未入力＝前の版を引き継ぐ）。
  const textChanges = {};
  for (const [k, n] of [['headline', 200], ['body', 4000], ['cta', 100]]) {
    const v = pick(k, n);
    if (String(v || '').trim()) textChanges[k] = v;
  }
  // ⚠️ ここでは止めない。人が「直して」と言った事実は、文言が空でも履歴に残す。
  //    ③へ送れるかどうかの判定は buildGenerateRequest（送る直前）で行う。
  return {
    ok: true,
    creative: {
      ...creative,
      ...
      headline: textChanges.headline !== undefined ? textChanges.headline : str(creative.headline, 200),
      body: textChanges.body !== undefined ? textChanges.body : str(creative.body, 4000),
