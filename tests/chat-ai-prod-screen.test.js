...
describe('本番画面: request_id の扱い', () => {
  ...
  // ①が修正済み（385a42a）。判定内容は②が未修正として記録したときと同じ＝
  // 「再試行が画面のボタンから呼べること」。通常の回帰テストとして常に通ること。
  it('🔴 再試行は画面から呼べる（aiTrialRetry がボタンに繋がっている）', () => {
    expect(html).toContain('onClick={aiTrialRetry}');
    expect((html.match(/aiTrialRetry/g) || []).length).toBeGreaterThan(1);   // 定義だけで終わっていない
  });
