    ...
    await parallelWithLimit(configs, 5, async (cfg, j) => {
      ...
      try {
        ...
        if (!r) {
          ...
          // 部分取得（サブスク/インボイスがタイムアウト）はキャッシュしない → 次回SYNCで再取得・完走を狙う
          const isPartial = r && r._diag && ['subscriptionsApi', 'invoicesApi', 'paymentsApi'].some(k => {
            const v = r._diag[k]; return v && (String(v).includes('部分結果') || String(v).includes('タイムアウト') || String(v).includes('timeout'));
          });
          if (r && !r._failed && !isPartial) setCachedAccount(j, r);
        }
        if (!r || r._failed) {
