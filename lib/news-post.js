...
const COVER_BY_CAT = Object.freeze({
  notice: { kicker: 'NOWL / BRIEFING', line1: 'HQ', line2: 'BRIEFING.', tone: 'grey' },
  event: { kicker: 'NOWL / EVENT', line1: 'LEARN', line2: 'TOGETHER.', tone: 'red' },
  rule: { kicker: 'NOWL / STANDARD', line1: 'NEW', line2: 'STANDARD.', tone: 'indigo' },
  study: { kicker: 'NOWL / STORIES', line1: 'KNOWLEDGE', line2: 'IN MOTION.', tone: 'dark' },
  case: { kicker: 'NOWL / CASES', line1: 'REAL', line2: 'RESULTS.', tone: 'amber' },
  praise: { kicker: 'NOWL / PEOPLE', line1: 'WELL', line2: 'DONE.', tone: 'pink' },
});
const COVER_DEFAULT = Object.freeze({ kicker: 'NOWL / UPDATES', line1: 'TEAM', line2: 'UPDATE.', tone: 'grey' });
 ...
 */
export function coverStyle(post) {
  const p = (post && typeof post === 'object') ? post : {};
  const imgId = coverOf(p);
  if (imgId) return { imgId, kicker: '', line1: '', line2: '', tone: '' };
  const base = COVER_BY_CAT[str(p.category)] || COVER_DEFAULT;
  return { imgId: '', ...base, tone: p.featured ? 'dark' : base.tone };
}
