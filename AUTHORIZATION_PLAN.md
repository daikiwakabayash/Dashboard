...
### 6-1. ✅ フロントからの認証情報の送信（**対応済み**）

**対応内容**（`feature/cc-foundation`）:

| 経路 | 実装 |
|---|---|
| SSOセッション | 既存の `window.fetch` インターセプタの対象に `/api/plan-store` を追加。`Authorization: Bearer` が自動付与される |
| パスワードアカウント（root / owner） | `ccPost` が `owner` と `token` を本文に添える。サーバーはハッシュ照合で確認する |

`/api/plan-store` は `Cache-Control: no-store` のため、`Authorization` を付けても
SalonOne のようなエッジキャッシュ失効の問題は起きない。

#### 🔴 これを入れる前に解決した問題: 上流のレート制限

`/api/plan-store` は **チャット6秒・掲示板8秒・プレゼンス30秒**で常時ポーリングされる。
`resolveActor` が Bearer を最初に試す設計のままだと、**ポーリングのたびに SalonOne の `/me` を呼ぶ**ことになり、
レート制限（60/分）を即座に超過して**ログインまで巻き添えで落ちる**。

→ 2つの対策を入れた（`lib/actor.js`）:

1. **確認順を入れ替え、ネットワークを使わない方法を先に試す**
   エージェントトークン → CRON_SECRET → rootトークン → オーナートークン → **SalonOne Bearer（最後）**
   root / owner はハッシュ照合だけで済むため、`/me` を一切呼ばない。
2. **Bearer 検証結果を60秒キャッシュする**
   同じトークンの連続リクエストで `/me` は1回しか呼ばれない（テストで固定）。

```
テスト: 同じBearerで10回連続 → /me の呼び出しは 1回
       rootトークン / cron / エージェント → /me は 0回
```

⚠️ **キャッシュの副作用**: トークンを失効させても**最大60秒**は有効なままになる。
`enforce` へ上げる前に、この60秒を許容できるか確認すること（§8-1 に追加）。
...
- [x] §6-1 のフロント側トークン付与が入っている（`feature/cc-foundation`）
- [ ] Bearer 検証キャッシュの60秒を許容できることを確認した（§6-1）
