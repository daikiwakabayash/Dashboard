...
const ACTION_PATTERNS = [
  ...
  /(に|へ)?(以下|下記|この|次)の?(文章|メッセージ|内容|件|話)?(を)?/g,
...
export function parseRecipientText(text) {
  ...
  for (const piece of pieces) {
    ...
    const plain = piece.replace(new RegExp(`${HINT_OPEN}\\d+${HINT_CLOSE}`, 'g'), '').trim();
    if (!plain) continue;
...
export function resolveRecipients(text, ctx) {
  ...
  return { intent, people: [...seen.values()], groups, ambiguous, unknown, total: seen.size };
}
...
/** そのまま送ってよいか。曖昧・未一致・0名が残っていれば止める。 */
export function canSend(res) {
  const r = res || {};
