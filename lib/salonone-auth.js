...
// Bearer を検証して { root, role, shopIds:[], shopNames:[], userId, staffId, loginId } を返す。無効なら null。
export async function verifySalonOneBearer(bearer) {
  ...
  try {
    ...
    return {
      ...
      // ⚠️ 画面は staff_id を本人IDに使う（無ければ user_id）。サーバーが user_id だけを見ると
      //    「本人なのに本人と判定されない」取り違えが起きるため、両方を持ち帰る。
      staffId: String(u.staff_id == null ? '' : u.staff_id),
      loginId: u.login_id || '',
    };
  } catch { return null; }
}
