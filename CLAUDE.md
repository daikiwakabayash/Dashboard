...
- **動画の送信前トランスコード（重要）**: iPhoneの動画はHEVC/.mov(高効率)で録画され受信側ブラウザの`<video>`で再生できないことがある（サムネイルは出るが再生停止＝LINE等と違い保存しないと見れない問題）。対策として `chatUploadFile` は動画を**送信前にH.264 MP4へ再エンコード**（`chatTranscodeVideo`＝キャンバスへ実時間drawImage＋`MediaRecorder`でキャプチャ、音声はWebAudio `createMediaElementSource`→`MediaStreamAudioDestinationNode`で合流、最大1280pxに縮小、`videoBitsPerSecond`2.5M）。出力形式は `chatPickVideoMime`（mp4録画可＝Safari/iOSはmp4／不可＝Chrome等はwebm。ただし元が既にmp4ならwebm化せず元使用＝iOSで再生不可のwebmを作らない）。**実時間変換のため進捗を`chatVideoConv`(0-100%)で表示**（「動画を変換中… N%」）。非対応端末/失敗時は `null` を返し**元ファイルをそのままアップロード**（安全側フォールバック）。再生モーダルは `onError`(`chatVideoErr`)で「この端末では再生できない形式です→端末に保存/別タブで開く」案内に切替（保険）。
