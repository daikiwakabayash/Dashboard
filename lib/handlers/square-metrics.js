...
// ── サブスク専用の永続キャッシュ（Vercel KV / Upstash REST）──
// SquareのサブスクAPIは遅く、毎回取得すると大型店（会員が多い）でタイムアウト＝部分取得になる。
// ACTIVE会員の次回課金日(chargedThroughDate)は月次でしか変わらないため、整形済みサブスク＋プランを
// KVに数時間キャッシュし、大半のSYNCではサブスク取得をスキップ＝高速化＆取りこぼし防止。
// invoices/payments（会員数・売上）は毎回フレッシュに取得（直近6ヶ月で高速）。
const SUBS_CACHE_TTL_MS = 3 * 60 * 60 * 1000; // 3時間
function _kvCreds() { const u = process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL || process.env.REDIS_REST_API_URL, t = process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN || process.env.REDIS_REST_API_TOKEN; return (u && t) ? { u, t } : null; }
function _hash(str) { let h = 2166136261 >>> 0; const s = String(str || ''); for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619) >>> 0; } return h.toString(36); }
async function kvGetSubs(key) {
  const c = _kvCreds(); if (!c) return null;
  try {
    const r = await fetch(`${c.u}/get/${encodeURIComponent(key)}`, { headers: { Authorization: `Bearer ${c.t}` } });
    if (!r.ok) return null;
...
async function kvSetSubs(key, value) {
  const c = _kvCreds(); if (!c) return;
  try {
    await fetch(`${c.u}/set/${encodeURIComponent(key)}?EX=${Math.round(SUBS_CACHE_TTL_MS / 1000)}`, {
      method: 'POST', headers: { Authorization: `Bearer ${c.t}`, 'Content-Type': 'text/plain' }, body: JSON.stringify(value),
    });
  } catch {}
}
...
async function fetchPlanDetails(client, planVariationIds, deadlineMs) {
  const plans = {};
  if (planVariationIds.length === 0) return plans;
  const uniqueIds = [...new Set(planVariationIds)];
  // ⚠️ plan詳細はサブスク取得後にチェーン実行されるため、ここで時間を使い過ぎると関数が60秒キルされ
  //    完走済みのinvoices（会員数の正データ源）ごと全損する。締切(既定8秒)を超えたら以降のバッチは諦める。
  const startTime = Date.now();
  const budget = deadlineMs || 8000;
  const perBatchTimeout = Math.min(8000, budget);

  for (let i = 0; i < uniqueIds.length; i += 100) {
    if (Date.now() - startTime > budget) {
      console.warn(`[fetchPlanDetails] 締切(${budget}ms)超過のため打ち切り (${Object.keys(plans).length}件取得済)`);
      break;
    }
    const batch = uniqueIds.slice(i, i + 100);
    try {
      // バッチ単位タイムアウト: 全体タイムアウトで部分結果を失わないよう上限を設ける
      let timer;
      const remaining = Math.max(1000, budget - (Date.now() - startTime));
      const { result } = await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = setTimeout(() => reject(new Error('Plan batch timeout')), Math.min(perBatchTimeout, remaining));
        }),
...
async function fetchSalesFromPayments(token, env, locationIds, diag, timeoutMs = 180000) {
  ...
  // 会員数/MRR/課金予定に必要なのは直近のみ（過去の深い履歴は取得しない＝大型店でも高速・完走）。
  // 6ヶ月前の月初を起点（当月＋直近5ヶ月＝前月比・短期トレンドに十分）。past分を切ることで
  // インボイス/決済のページ数が激減し、千葉等の大型店でもタイムアウト(部分取得)しなくなる。
...
async function fetchInvoices(client, locationIds, diag, timeoutMs = 180000, lookbackMonths = 13) {
  const errors = [];
  const allInvoices = [];
  const startTime = Date.now();
  let timedOut = false;
  // 過去履歴の打ち切り（ソートDESC）。会員数/MRR/前月比は直近数ヶ月で十分だが、
  // コホートLTV/継続分析は12ヶ月分のコホートを追うため直近13ヶ月まで取得する。
  // ?months=N（=lookbackMonths）で遡及を拡張可能（LTV等を過去から正確に算出する用途・既定13）。
  // ⚠️ 大型店で件数が増えると重くなる（タイムアウト時は部分取得＝直近優先で確保）。
  const _lb = Number(lookbackMonths) > 0 ? Math.min(72, Number(lookbackMonths)) : 13;
  const cutoffDate = new Date();
  cutoffDate.setMonth(cutoffDate.getMonth() - _lb);
...
async function fetchAccountData(tokenConfig, isMultiAccount) {
  ...
  try {
    ...
    // ⚠️ Vercel(Hobby)の maxDuration は 60秒。これを超えると関数が「ハードキル」され、
    //    取得済みデータごと全損→リトライになる。そのため内部タイムアウトは60秒より十分手前(48秒)に置き、
    //    時間切れでも「部分結果」を確実に返す（全損より過少表示の方が復旧しやすい）。maxDurationを上げるならここも上げる。
    const API_TIMEOUT = 48000; // 各API最大48秒（60秒キルの手前で部分結果を返す）
    // ⚠️⚠️ 会員数・MRR・店舗別売上は「invoices/payments（決済実績）」が唯一の正データ源であり、
    //    これらは大型店でも6ヶ月カット済みで完走する。subscriptions は未来予定決済(chargedThroughDate)・
    //    解約率・プラン内訳の副次情報にすぎない。ところがサブスクAPIは非常に遅く(新宿=600件で52s)、
    //    plan詳細がサブスク後にチェーンするため、SUBS_TIMEOUT=52秒だと 52s+plan+直列化 が 60秒キルを超え、
    //    「完走済みのinvoicesごと全損」→ 会員数が激減表示（新宿=実120名なのに5名）していた。これが数字不整合の主因。
    //    → サブスクは40秒で確実に打ち切り、plan詳細も締切付きにして、関数は必ず ~50秒以内に完走させる。
    //       サブスクが部分的でも会員数(invoices基準)は完全・正確に出る。完全なサブスクが必要ならKVキャッシュを構成する。
    const SUBS_TIMEOUT = 40000; // サブスクは40秒で打ち切り、正データ源のinvoicesを確実に返す（60秒キルで全損させない）
    const PLAN_DETAILS_DEADLINE_MS = 8000; // サブスク取得後のplan詳細は最大8秒（合計 40+8=48秒 で60秒キルの手前）
    // サブスク・インボイスは内部で部分結果を管理するため withTimeout を使わない
    // プラン詳細はサブスクデータに依存するため、サブスク取得完了後に即座にチェーン実行
    // （他APIの完了を待たずに並列で開始することで10-30秒短縮）
    // サブスク永続キャッシュ（KV）: 直近3時間の整形済みサブスクがあれば取得をスキップ（高速・取りこぼし防止）。
    // ?freshsubs=1 または tokenConfig.forceSubs で強制再取得（深いSYNC用）。
    const subsKey = `sq:subs:v1:${_hash(tokenConfig.token)}:${_hash(locationIds.join(','))}`;
    const cachedSubs = tokenConfig.forceSubs ? null : await kvGetSubs(subsKey);
    const cacheFresh = !!(cachedSubs && Array.isArray(cachedSubs.subscriptions) && (Date.now() - (cachedSubs.ts || 0) < SUBS_CACHE_TTL_MS));
    // 取得スキップは「完走済みキャッシュ」がある時だけ。部分キャッシュ（大型店）は毎回再取得してunionで補完し、
    // 完走したらcomplete=trueで保存→以後スキップ。これで仙台/千葉のような大型店も数回のSYNCで取りこぼしゼロになる。
    const subsFromCache = cacheFresh && cachedSubs.complete === true;

    let plans = {};
    const subsTask = subsFromCache
      ? Promise.resolve(null)   // キャッシュ利用時はサブスク取得をスキップ（遅いAPIを回避）
      : fetchAllSubscriptions(client, locationIds, diag, SUBS_TIMEOUT).then(async (subs) => {
          const planVariationIds = subs.map(s => s.planVariationId).filter(Boolean);
          plans = await fetchPlanDetails(client, planVariationIds, PLAN_DETAILS_DEADLINE_MS);
          return subs;
        });
    const [rawSubs, invoices, paymentsData, customers] = await Promise.all([
      subsTask,
      fetchInvoices(client, locationIds, diag, API_TIMEOUT, tokenConfig.lookbackMonths),
      fetchSalesFromPayments(tokenConfig.token, tokenConfig.env, locationIds, diag, API_TIMEOUT),
      withTimeout(fetchAllCustomers(client, diag), API_TIMEOUT, {}, 'Customers'),
    ]);
    if (subsFromCache) plans = cachedSubs.plans || {};

    // タイムアウト検出: withTimeoutがfallbackを返した場合、diagに記録がない
    // Note: subscriptionsApi/invoicesApiは各関数内部で設定済み
    if (!diag.subscriptionsApi) diag.subscriptionsApi = 'timeout (48s)';
    if (!diag.invoicesApi) diag.invoicesApi = 'timeout (48s)';
    if (!diag.paymentsApi) diag.paymentsApi = 'timeout (48s)';
    if (!diag.refundsApi) diag.refundsApi = 'timeout (48s)';
    if (!diag.customersApi) diag.customersApi = 'timeout (48s)';

    const payments = paymentsData.payments || [];
    const refunds = paymentsData.refunds || [];
    // キャッシュ利用時は rawSubs=null（サブスク取得スキップ）。プラン診断はフレッシュ取得時のみ。
    if (!subsFromCache) {
    ...
    // サブスクリプションを整形（キャッシュ利用時は保存済みの整形結果をそのまま使用）
    let subscriptions;
    if (subsFromCache) {
      subscriptions = cachedSubs.subscriptions;
      diag.subscriptionsApi = `cache: ${subscriptions.length}件 (${Math.round((Date.now() - cachedSubs.ts) / 60000)}分前・KV)`;
    } else {
      const freshMapped = rawSubs.map(s => {
      ...
      const subsDiag = String(diag.subscriptionsApi || '');
      const wasPartial = subsDiag.includes('部分結果') || subsDiag.includes('タイムアウト') || subsDiag.includes('エラー');
      // 部分取得（大型店）は前回キャッシュとunion（id単位・新しい方優先）して取りこぼしを補完。
      // これでこのレスポンス自体も前回分を含んだ完全な人数になり、保存すれば数回で完成する。
      subscriptions = freshMapped;
      if (wasPartial && cacheFresh && Array.isArray(cachedSubs.subscriptions)) {
        const byId = new Map();
        for (const s of cachedSubs.subscriptions) byId.set(s.id, s);
        for (const s of freshMapped) byId.set(s.id, s); // 今回取得を優先（最新の状態/次回課金日）
        subscriptions = [...byId.values()];
        if (cachedSubs.plans) plans = { ...cachedSubs.plans, ...plans };
        diag.subscriptionsApi = `${subsDiag} + 前回分union=${subscriptions.length}件`;
      }
      // 保存: 完走なら complete=true（以後スキップ）／部分でも union を保存し次回さらに補完（過少データを固定化しない）。
      if (subscriptions.length > 0) {
        await kvSetSubs(subsKey, { ts: Date.now(), subscriptions, plans, complete: !wasPartial });
      }
    }
...
// 数値の正確性に直結するAPIが部分結果（タイムアウト）のときはキャッシュを抑止し、次回SYNCで取り直す。
// invoices/payments/refunds = 会員数・MRR・総売上。
// subscriptions = 未来月の会員数・課金予定（chargedThroughDate）の唯一の源。過去分カットで予算に余裕が
//   できたため通常は完走するが、万一途中打ち切りになった場合に過少データを固定化しないよう critical 扱いにする。
function isCriticalPartial(r) {
  if (!r || !r._diag) return false;
  return ['invoicesApi', 'paymentsApi', 'refundsApi', 'subscriptionsApi'].some(k => {
    const v = r._diag[k];
    return v && (String(v).includes('部分結果') || String(v).includes('タイムアウト') || String(v).includes('timeout'));
  });
}
...
export default async function handler(req, res) {
  ...
  // ?freshsubs=1 でサブスクの永続キャッシュを無視して強制再取得（深いSYNC）
  const freshSubs = !!(req.query && req.query.freshsubs === '1');
  if (configs.length === 0) {
  ...
  if (req.query && req.query.accounts !== undefined) {
    ...
    const allDiag = [];
    // ?months=N: LTV等を過去から正確に算出するため取得遡及を拡張（既定13）。深い取得はキャッシュを介さない（13ヶ月キャッシュ汚染を防止）。
    const lookbackMonths = Math.min(72, Math.max(1, parseInt((req.query && req.query.months), 10) || 13));
    const deepFetch = lookbackMonths > 13;

    // バッチ内アカウントを並列取得（最大3並列、1アカウント最大100秒タイムアウト）
    await parallelWithLimit(indices, 3, async (idx) => {
      const cfg = configs[idx];
      try {
        // キャッシュ確認（深い遡及取得時はキャッシュをバイパス）
        let r = deepFetch ? null : getCachedAccount(idx);
        if (!r) {
          // アカウント単位タイムアウト: バッチ全体を守る
          let batchTimer;
          r = await Promise.race([
            fetchAccountData({ ...cfg, forceSubs: freshSubs, lookbackMonths }, true),
            new Promise((_, reject) => {
              batchTimer = setTimeout(() => reject(new Error('アカウント取得タイムアウト (280秒)')), 280000);
            }),
          ]).finally(() => clearTimeout(batchTimer));
          // invoices/payments が部分結果のアカウントはキャッシュしない（正確な数値を凍結させない）。深い遡及もキャッシュしない。
          if (r && !r._failed && !isCriticalPartial(r) && !deepFetch) setCachedAccount(idx, r);
        }
        if (!r || r._failed) {
  ...
  if (req.query && req.query.account !== undefined) {
    ...
    try {
      ...
      if (!r) {
        let singleTimer;
        r = await Promise.race([
          fetchAccountData({ ...cfg, forceSubs: freshSubs }, configs.length > 1),
          new Promise((_, reject) => {
            singleTimer = setTimeout(() => reject(new Error('アカウント取得タイムアウト (280秒)')), 280000);
          }),
        ]).finally(() => clearTimeout(singleTimer));
        if (r && !r._failed && !isCriticalPartial(r)) setCachedAccount(idx, r);
      }
  ...
  const deadlineTimer = setTimeout(() => { deadlineReached = true; }, 54000); // 60秒キルの手前で打ち切り
  ...
  try {
    ...
    await parallelWithLimit(configs, 5, async (cfg, j) => {
      ...
      try {
        ...
        if (!r) {
          r = await fetchAccountData({ ...cfg, forceSubs: freshSubs }, isMultiAccount);
          // invoices/payments がタイムアウト（部分結果）のアカウントはキャッシュしない → 次回SYNCで完走を狙う
          // サブスクのみ部分の場合はキャッシュOK（会員数/金額はインボイス基準で完全なため）
          if (r && !r._failed && !isCriticalPartial(r)) setCachedAccount(j, r);
        }
        if (!r || r._failed) {
