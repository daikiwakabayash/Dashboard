 ...
 */
export function analyzeStore(store, th = PATROL_THRESHOLDS) {
  ...
  if (newPct != null) {
    if (newPct <= -th.newDropPct) push('warn', 'new_drop', `新規来店が前月比 ${newPct}%（${prev.newCount}→${cur.newCount}名）`,
      'クーポンの見直し・予約枠の空き確認・サムネ画像の刷新を。ホットペッパー/Googleの初回オファーが競合に負けていないか点検を。');
    else if (newPct >= th.newGrowPct) push('good', 'new_grow', `新規来店が前月比 +${newPct}%（${prev.newCount}→${cur.newCount}名）好調`,
      '好調要因（媒体・クーポン・スタッフ）を言語化し、他店へ横展開を。');
  }

  // ── 総売上トレンド ──
  const salesPct = pctChange(cur.gross, prev.gross);
  if (salesPct != null && salesPct <= -th.salesDropPct) push('warn', 'sales_drop',
    `総売上が前月比 ${salesPct}%（${yen(prev.gross)}→${yen(cur.gross)}）`,
  ...
  // ── 新規来店の絶対数 ──
  if (hasData && (cur.newCount || 0) > 0 && (cur.newCount || 0) < th.newCountLow) push('info', 'new_few',
    `新規来店 ${cur.newCount}名/月（${th.newCountLow}名未満）`,
