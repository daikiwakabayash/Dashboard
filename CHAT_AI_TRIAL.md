...
## 通常チャット入力欄からの @AI（②が追加・本部/root・フラグON・検証Roomのみ）

管理タブの検証画面と同じサーバー経路（`?type=chatai`）を、**通常チャットの入力欄**からも使えるようにしました。

- 起動は従来どおり「🤖 AIに質問」ボタン、または本文の `@AI`。**新しいUIは足していません。**
- 対象Roomの判定は**サーバーが返す `targets`** をそのまま使います（画面で同じ判定を書き直しません）。
  `cc_ai_trial` が OFF、または root 以外、または検証Room以外では**従来の動きのまま**です。
- 新しい送信には新しい `request_id`。**同じ送信の再試行だけ**同じIDを使い回します
  （入力欄の編集に引きずられないよう、再試行は送信時の質問・Roomを使います）。
- 失敗表示の隣に「**同じ送信を再試行**」を出します。サーバーが `retryable:false`（権限エラー等）
  と返した失敗では出しません。実行中は二重に走りません。

### 画面レベルの確認（`scripts/chat-composer-check.mjs`）

本物の `index.html` を headless Chromium で操作し、入力欄から送ったときに何が飛ぶかを観測します
（ローカルのスタブAPIに対してのみ・**本番には接続しません**）。

| # | 確認 | 結果 |
|---|---|---|
| A | チャットに検証Roomが出る | ✅ |
| 1 | 検証Roomの @AI は `?type=chatai` へ行く | ✅ `room_id=g_trial` |
| 2 | 従来の `/api/chat` は使わない | ✅ 0回 |
| 3 | 新しい送信には新しい `request_id` | ✅ `req_…kc9u04` → `req_…blsnx9` |
| 4 | 回答が同じRoomに表示される | ✅ |
| 5 | 検証Room以外は新経路に変えない | ✅ 従来の経路のまま |
