...
export function normalizeActor(raw) {
  ...
  return {
    ...
    // 同一人物の別ID（SSOのstaff_id/user_id、本部アカウントに紐付けた staffId）。
    // これが無いと、DMやグループの本人判定が取り違えで落ちる。
    altIds: Array.isArray(a.altIds) ? a.altIds.filter(v => v != null && v !== '').map(s => String(s)).slice(0, 10) : [],
...
export const ACTIONS = Object.freeze({
  ...
  // ⚠️ AIエージェントは自由な送信を行えない（agentAllowed:false）。
  //    AIがチャットに書き込む経路は下の2つに限る。どちらも「宛先をAIが選ばない」もの。
  'chat.send':          { minRank: 20, humanOnly: false, agentAllowed: false, scoped: true  },
  // 人が @AI で呼んだルームに、その場で返信する（宛先＝呼ばれたルーム。AIは選べない）
  'chat.ai_reply':      { minRank: 20, humanOnly: false, agentAllowed: true,  scoped: true  },
  // AIパトロールが店舗ルームへ投稿する（本部が起動したときのみ。宛先＝その店舗）
  'chat.patrol_post':   { minRank: 90, humanOnly: false, agentAllowed: true,  scoped: true  },
...
// ── 一括送信の追加確認しきい値（役割別・テナントで上書き可）──────────────
// 「何名から確認を出すか」は運用の実感で決まる数字なので、コードに固定しない。
// テナント設定（将来のホワイトラベル）で上書きできるようにしておく。
export const DEFAULT_BULK_THRESHOLDS = Object.freeze({
  staff: 20, manager: 20,     // 現場は誤操作の影響が読みにくいので低め
  owner: 50,                  // 管轄店舗をまとめて送る運用があるため中間
  admin: 100, root: 100,      // 本部は日常的に広く送るため高め
  guest: 20,
});

// テナント設定から役割別のしきい値を引く。未設定なら既定値。
//   config = { bulkThresholds: { staff: 30, owner: 80, … } }
export function bulkThreshold(actor, config) {
  const a = normalizeActor(actor);
  const c = (config && typeof config === 'object' && config.bulkThresholds) || {};
  const v = Number(c[a.role]);
  return Number.isFinite(v) && v > 0 ? Math.floor(v) : DEFAULT_BULK_THRESHOLDS[a.role];
}

// 「全社送信」「全店舗送信」は人数に関わらず最も強い確認を出す。
// 人数が少なくても、届く範囲が組織全体なら取り返しがつかないため。
export function isOrgWideSend(action, target) {
  const t = (target && typeof target === 'object') ? target : {};
  if (String(action) === 'chat.broadcast_all') return true;
  if (t.allShops === true) return true;                       // 「全店舗へ」と明示された
 ...
 * 送信前にどの強さの確認を出すか。
 *   'none'    確認なしで送信ボタン
 *   'confirm' 人数を明示した追加確認（1段）
 *   'strong'  全社・全店舗向けの最終確認（人数に関わらず必ず出す）
 */
export function confirmLevel(action, target, actor, config) {
  if (isOrgWideSend(action, target)) return 'strong';
  const t = (target && typeof target === 'object') ? target : {};
  const n = Number(t.recipientCount) || 0;
  return n >= bulkThreshold(actor, config) ? 'confirm' : 'none';
}

// ── ルーム種別ごとの可視・投稿の判定 ────────────────────────────────
// PR #383 の lib/chat-authz.js が持っていた規則を、判断源をひとつに保つため
// ここへ統合したもの（第二の権限システムを作らない）。
//   room = { id, kind:'announce'|'store'|'group'|'dm', shop, members:[staffId], status }
export const ROOM_KINDS = Object.freeze(['announce', 'store', 'group', 'dm']);

function memberIds(room) {
  const r = (room && typeof room === 'object') ? room : {};
  return (Array.isArray(r.members) ? r.members : []).map(String);
}
// 本人IDの候補（SSOのstaff_id/user_id、本部アカウントに紐付けた staffId の取り違えを防ぐ）
function actorIds(actor) {
  const a = normalizeActor(actor);
  const ids = [a.id, ...(Array.isArray(a.altIds) ? a.altIds : [])].map(String).filter(Boolean);
  return ids;
}
export function isRoomMember(actor, room) {
  const mine = actorIds(actor);
  const mem = memberIds(room);
  return mine.some(id => mem.includes(id));
}

// そのルームが見えるか。
export function canViewRoom(actor, room) {
  const a = normalizeActor(actor);
  const r = (room && typeof room === 'object') ? room : null;
  if (!r) return false;
  // テナントが違えば、役割に関わらず見えない
  if (r.tenantId && normTenant(r.tenantId) !== a.tenantId) return false;
  const kind = String(r.kind || '');
  // DM は当事者だけ。root / 本部でも覗けない（ここを開けるとチャット自体が使われなくなる）
  if (kind === 'dm') return isRoomMember(a, r);
  // グループはメンバーのみ。ただし root / 本部は運営上の必要から見える
  if (kind === 'group') return isRoomMember(a, r) || !ROLES[a.role].scoped;
  // 全社アナウンスは全員が読める
  if (kind === 'announce') return roleRank(a) > 0;
  // 店舗ルームは、その店舗の管轄者のみ（root / 本部は全店）
  if (kind === 'store') return inScope(a, r.shop);
  return false;
}
...
export function canPostRoom(actor, room) {
  const a = normalizeActor(actor);
  const r = (room && typeof room === 'object') ? room : null;
  if (!r) return { allow: false, code: 'no_room', reason: 'ルームがありません' };
  if (!canViewRoom(a, r)) return deny('room_not_visible', 'このルームは表示できません');
  const status = String(r.status || 'active');
  if (status !== 'active') return deny('room_readonly', 'このルームは閲覧のみです');
  // 全社アナウンスへ投稿できるのは本部以上のみ
  if (String(r.kind) === 'announce' && roleRank(a) < ROLES.admin.rank) {
    return deny('announce_hq_only', '全社アナウンスに投稿できるのは本部のみです');
  }
  // AIエージェントは、呼ばれた返信とパトロール以外では投稿しない
  if (a.source === 'agent') return deny('agent_forbidden', 'AIエージェントは投稿できません');
...
export function can(actor, action, target) {
  ...
  // 10) ルームが指定された操作は、そのルームの種別規則も満たすこと
  //     （全社アナウンスは本部のみ・DMは当事者のみ・アーカイブ済みは読み取り専用）
  if (t.room && String(action).startsWith('chat.')) {
    const rp = canPostRoom(a, t.room);
