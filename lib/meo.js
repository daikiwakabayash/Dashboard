...
// Placesが返す直近レビュー（最大5件）の publishTime から「今月分」を数える。
//   スナップショット履歴が無い初回でも、当月に投稿された口コミの下限値を出すための即時推定。
//   capped=true は5件すべてが当月＝「5件以上」の可能性（Places APIは最大5件しか返さないため上限で頭打ち）。
export function reviewsInMonth(reviews, now = new Date()) {
  const list = Array.isArray(reviews) ? reviews : [];
  const ym = jstYmd(now).slice(0, 7);
  let count = 0;
  for (const r of list) {
    const t = String((r && (r.publishTime || r.when)) || '');
    // publishTime は RFC3339(UTC)。JSTの年月に合わせて判定。
    const d = Date.parse(t);
    if (!isNaN(d) && jstYmd(new Date(d)).slice(0, 7) === ym) count++;
  }
  return { count, capped: list.length >= 5 && count >= 5 };
}
