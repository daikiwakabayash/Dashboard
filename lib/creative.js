...
export function normalizeFile(raw, opts = {}) {
  ...
  const sha256 = /^[0-9a-f]{64}$/.test(str(f.sha256, 64)) ? str(f.sha256, 64) : '';
  return {
    // 保存先URLは **サーバー側だけ**が持つ。画面へ出さない。
    storageUrl,
    // 画面が使う参照。①の認証付きproxyがこのIDでbyteを返す。
...
// 画面・③へ渡す形。**storageUrl を落とす。**
export function publicFile(f, creativeId) {
  if (!f) return null;
  return {
    fileId: f.fileId, kind: f.kind, contentType: f.contentType, bytes: f.bytes,
    sha256: f.sha256 || null, name: f.name,
    // 認証付きproxyのパス。ブラウザはこれだけを見る。
    src: `/api/plan-store?type=creative&action=asset&creativeId=${encodeURIComponent(creativeId || '')}&fileId=${encodeURIComponent(f.fileId)}`,
  };
}
export function publicFiles(files, creativeId) {
  return arr(files).map(f => publicFile(f, creativeId)).filter(Boolean);
}
