...
async function fetchAccountData(tokenConfig, isMultiAccount) {
  ...
  try {
    ...
    // ⚠️ 会員数・MRR・総売上は「インボイス/決済」基準（payersInMonths）で算出するため、
    //    アカウント全体の所要時間は「最も遅いAPI」で決まる Promise.all の性質上、
    //    最重量・タイムアウトしがちな subscriptions が全体を引きずる。
    //    subscriptions は解約内訳・新規開始日などの副次情報にしか使わないので、
    //    専用に短いバジェット(24秒)を与えて重い店舗を早く完走させる。
    //    正確性が必須の invoices/payments/customers はフルの48秒を確保。
    const SUBS_TIMEOUT = 24000; // サブスクは副次データ→半分の予算で早期打ち切り（会員数/金額はインボイス基準で不変）
