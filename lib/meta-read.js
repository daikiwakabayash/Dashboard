...
export function normalizeMetric(raw, key, currency) {
  ...
  const resolvedUnit = normalizeUnit(m.unit, currency);
  ...
  return {
    key: str(key, 40),
    value,
    unit: unitOk ? resolvedUnit : (accountCurrencyOf(currency) || 'count'),
 ...
 */
export function normalizeUnit(raw, currency) {
...
export function looksLikeSample(raw) {
  ...
  if (typeof d.mode === 'string' && /^(sample|mock|fixture|demo|test)$/i.test(d.mode)) return true;
 ...
 *   'unstated'    … そもそも申告が無い（旧形式。従来どおり扱う）
 * ⚠️ **'unconfirmed' を live へ昇格させない。** 確かめられない数字は実績として見せない。
 */
export function declaredMode(raw) {
  const d = (raw && typeof raw === 'object') ? raw : {};
  if (looksLikeSample(d)) return 'sample';
  const stated = [d.mode, d.data_mode].find(v => typeof v === 'string' && v.trim() !== '');
  if (stated === undefined) return 'unstated';
  return /^live$/i.test(stated.trim()) ? 'live' : 'unconfirmed';
}
