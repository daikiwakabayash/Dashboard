...
async function loadAccounts() {
  ...
  const metaMap = {};
  // GAS(オーナー設定)とKV(パスワード/拡張情報)は独立なので並列取得してレイテンシを短縮。
  const useKv = kvConfigured();
  const [gas, kvPass, kvMeta] = await Promise.all([
    loadGasOwners(),
    useKv ? kvBlobGet(ACCT_PASS_KEY).catch(() => ({})) : Promise.resolve(null),
    useKv ? kvBlobGet(ACCTMETA_KEY).catch(() => ({})) : Promise.resolve(null),
  ]);
  for (const o of gas) {
  ...
  if (kvPass && typeof kvPass === 'object') {
...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    // ── 本部(hq)ログインの高速化: KVだけで判定できる場合はGAS(オーナー設定)往復をスキップ。 ──
    //    UIで作成したアカウントはパスワード(ACCT_PASS_KEY)・拡張情報(ACCTMETA_KEY)ともにKVにあるため、
    //    hqは shops=null（全店）で完結する＝KVのみで即応答できる。KVに無ければ下のGAS経路にフォールバック。
    if (b.action === 'login' && b.owner && b.password != null && kvConfigured()) {
      try {
        const [kvPass, kvMeta] = await Promise.all([kvBlobGet(ACCT_PASS_KEY), kvBlobGet(ACCTMETA_KEY)]);
        const pw = kvPass && kvPass[b.owner];
        const meta = (kvMeta && kvMeta[b.owner]) || null;
        if (pw != null && String(pw) === String(b.password) && meta && meta.role === HQ_ROLE) {
          return res.status(200).json({ ok: true, owner: b.owner, root: true, token: rootToken(), shops: null, role: HQ_ROLE, staffId: meta.staffId || '', staffName: meta.staffName || '' });
        }
      } catch (_) {}
    }
  }
