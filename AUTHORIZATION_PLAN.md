...
3. **失敗した検証も60秒キャッシュする**
   期限切れ・無効トークンで `/me` が失敗した場合も結果（＝本人確認できなかった）をキャッシュする。
   ここを素通しにすると、**無効なトークン1本がポーリングし続けるだけで60回/分を使い切る**。
   キャッシュに当たっても返すのは `verified:false` の主体なので、誤って許可されることはない。

---

#### ⚠️ 60秒キャッシュのリスクと、高リスク操作の再検証（ユーザー要求により明記）

**リスクの中身**

Bearer の検証結果を60秒キャッシュするため、**トークンを失効させてから最大60秒間、
そのトークンが有効なものとして通ってしまう**。

| 起きること | 最大の遅れ | 具体例 |
|---|---|---|
| 退職者のアカウントを止めた | 60秒 | 退職処理の直後60秒だけチャットを送れる |
| 役割を staff へ降格した | 60秒 | 降格前の広い権限で60秒だけ操作できる |
| 管轄店舗を減らした | 60秒 | 外した店舗のデータを60秒だけ見られる |
| トークンが漏れて失効させた | 60秒 | 攻撃者が60秒だけ操作できる |

**なぜ60秒を受け入れるか**

キャッシュを外すと、チャット6秒ポーリング × 利用者数だけ `/me` を呼ぶことになり、
SalonOne の 60回/分を即座に超える。超えると**ログインを含む全機能が落ちる**。
「60秒の権限の遅れ」と「全機能停止」を比べ、前者を選んでいる。

**高リスク操作だけキャッシュを使わない（実装済み）**

取り返しがつかない操作に限り、キャッシュを迂回して**毎回 `/me` を引き直す**。
対象は `lib/authz.js` の `REVERIFY_ACTIONS`、判定は `needsReverify(action)`。
`api/plan-store.js` が `resolveActor(req, { skipCache: needsReverify(action) })` として渡す。

| 区分 | 操作 | キャッシュ | 失効の反映 |
|---|---|---|---|
| 再検証（毎回 `/me`） | 承認 `approval.approve` / 実行 `approval.execute` | 使わない | 即座 |
| 〃 | フラグ変更 `flag.change` / キルスイッチ解除 `killswitch.release` | 使わない | 即座 |
| 〃 | 広告 `meta.budget_change` `meta.pause` `meta.resume` `meta.creative_replace` | 使わない | 即座 |
| 〃 | 外向き `sns.post` `lp.change` `knowledge.publish` | 使わない | 即座 |
| 〃 | 全社一斉送信 `chat.broadcast_all` | 使わない | 即座 |
| 通常（60秒キャッシュ） | 閲覧・チャット送信・DM・既読など | 使う | 最大60秒 |

再検証の対象は**数が少なく、ポーリングされない操作**に限っているため、レート制限への影響はない
（1日あたり数十回のオーダー）。

**さらに縮めたい場合の案（未実装・人の判断が必要）**

| 案 | 効果 | 代償 |
|---|---|---|
| A. TTLを60秒→15秒へ短縮 | 遅れが最大15秒 | `/me` が4倍。利用者15人で上限に届く |
| B. 失効リスト（denylist）をKVに置き、キャッシュ利用前に確認 | 失効は即座に反映 | KV読み取りが1回増える。失効操作の実装が必要 |
| C. ログアウト時にフロントから失効を通知 | 通常のログアウトは即座 | 悪意ある利用者は通知しないので効果なし |

→ **推奨は B**。`enforce` へ上げる時点で必要かを判断する（§8-1）。
...
- [ ] Bearer 検証キャッシュの60秒を許容できることを確認した（§6-1）
      ※ 高リスク操作は既に再検証済み。残るのは閲覧・チャット送信などの通常操作のみ
- [ ] 60秒を許容できない場合、§6-1 の案B（失効リスト）を実装した
- [ ] `log` モードの記録に **ALLOW も DENY も**残っていることを確認した（実態把握のため）
