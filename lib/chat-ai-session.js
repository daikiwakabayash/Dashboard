...
//   - request_id は**送信のたびに新しく発行**する（質問本文＋ルームからの固定IDは作らない）。
//     同じ送信を**再試行**するときだけ同じ request_id を維持する。
//     → 同じ本文でも「別々に新規質問した」なら別の依頼として扱う（別タブでも同じ）。
//     同じ request_id の重複実行防止はサーバー（①）、回答カードの重複表示防止はここ（②）。
//   - 失敗しても質問と下書きを消さない。
...
// 送信1回ごとの依頼ID。**本文・ルームからは作らない**（作ると別々の新規質問が同じ依頼に潰れる）。
// 再試行のときは発行し直さず、送信時に発行したものを持ち回す。
export const newRequestId = () => genId('req');
...
export function createSession(input = {}) {
  return {
    ...
    requestIds: {},              // questionId → request_id（送信時に1回だけ発行・再試行で維持）
...
export function submit(state, opts = {}, t) {
  ...
  const message = {
    ...
    // 送信のたびに新しい依頼ID。opts.requestId は「同じ送信を復元する」場合のみ渡す
    // （例: 別タブが同じ送信レコードを共有して再試行する）。本文からは作らない。
    requestId: intent.ai ? (str(opts.requestId) || newRequestId()) : '',
  };
  ...
  next.pending = { ...next.pending, [message.id]: { startedAt: at, clientIds: [message.clientId].filter(Boolean), requestId: message.requestId } };
  next.requestIds = { ...(next.requestIds || {}), [message.id]: message.requestId };
  return {
    state: next,
    posted: message,
    aiRequest: {
      questionId: message.id, roomId: state.roomId, question: text,
      askedBy: state.viewerId, requestId: message.requestId, attempt: 1,
    },
  };
}
...
export function retry(state, questionId, t) {
  ...
  // 同じ送信の再試行 → 送信時に発行した request_id をそのまま使う（新規発行しない）。
  // 古い状態や別タブから復元した質問で欠けている場合だけ、ここで1回発行して固定する。
  const requestId = str((state.requestIds || {})[qid]) || str(q.requestId) || newRequestId();
  const messages = q.requestId === requestId ? state.messages
    : state.messages.map(m => (m.id === qid ? { ...m, requestId } : m));
  return {
    state: {
      ...state, errors, messages,
      requestIds: { ...(state.requestIds || {}), [qid]: requestId },
      pending: { ...state.pending, [qid]: { startedAt: now(t), clientIds: [], requestId } },
    },
    aiRequest: {
      questionId: qid, roomId: state.roomId, question: q.text,
      askedBy: q.fromStaffId, requestId, attempt: 'retry',
    },
  };
}
