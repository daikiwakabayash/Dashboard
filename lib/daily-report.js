...
export function buildPrepReport(input) {
  ...
  const card = {
    ...
    channels: {
      ...
      // 🔴 列の定義はカード側で持つ。描く側に項目名を当てさせない
      //    （期間ごとに列が違うので、当てさせると別の期間で全部「未取得」になる）。
      cols: [
        { key: 'added', label: '新規 / 件' },
        { key: 'cancelled', label: '取消 / 件' },
...
export function buildOpenReport(input) {
  ...
  const card = {
    ...
    channels: (() => {
      const rows = arr(chs).map(c => ({ name: str(c && c.name) || '未設定',
        booking: pick(c, 'booking'), visit: pick(c, 'visit'), join: pick(c, 'join') }));
      const col = (k) => sumStrict(rows.map(r => r[k]));
      return {
        header: '媒体別（新規予約）', note: ymdLabel(ymd) || ymd,
        cols: [
          { key: 'booking', label: '予約 / 件' },
          { key: 'visit', label: '来店 / 名' },
          { key: 'join', label: '入会 / 名', strong: true },
        ],
        rows,
        total: rows.length ? { booking: col('booking'), visit: col('visit'), join: col('join') } : null,
      };
    })(),
...
// ── リピート（次回予約）を新規客台帳から読む ────────────────────────────
// オーナーの説明:
//   マーケティング → 新規管理 の「新規客台帳」は、1人1行で 1回目・2回目… と並ぶ。
//   1回目に来店すると日時と金額が入り、次回予約を取っていれば 2回目の欄に「次回予約」が入る。
//   2回目が空＝次回予約なし＝リピートしていない（画面上部の「離反 / 次回予約なし」と同じ）。
//   この台帳は marketing/new-customers が元。
//
// 🔴 未確認（AGENTS.md §2）: 「次回予約があるか」を表す**項目名は確認できていない**。
//   そこで、ありそうな形をいくつか受ける。どれにも当たらなければ null（＝未取得）にして、
//   0人とは書かない。実際の項目名が分かれば、下のキー配列に足すだけでよい。
const NEXT_BOOL_KEYS = ['has_next_reservation', 'has_next_booking', 'next_reservation', 'has_next_visit'];
const NEXT_AT_KEYS = ['next_reservation_at', 'next_booking_at', 'next_visit_at', 'next_appointment_at'];
const CHURN_BOOL_KEYS = ['is_churn', 'churned', 'is_churned'];
const CHURN_TEXT_KEYS = ['continuation_status', 'retention_status', 'status_label'];
const VISIT_LIST_KEYS = ['visits', 'appointments', 'reservations', 'visit_history'];

// 1人ぶんの「次回予約を取れたか」。true / false / null（分からない）。
export function repeatStateOf(row) {
  const r = (row && typeof row === 'object') ? row : null;
  if (!r) return null;
  for (const k of NEXT_BOOL_KEYS) {
    if (typeof r[k] === 'boolean') return r[k];
  }
  for (const k of NEXT_AT_KEYS) {
    if (k in r) return !!(r[k] && str(r[k]).trim());     // 日時が入っていれば予約あり
  }
  for (const k of CHURN_BOOL_KEYS) {
    if (typeof r[k] === 'boolean') return !r[k];         // 離反＝次回予約なし
  }
  for (const k of CHURN_TEXT_KEYS) {
    const v = str(r[k]).trim();
    if (v === '継続') return true;
    if (v === '離反') return false;
  }
  // 1回目・2回目…の並びが配列で来る形。2件目があればリピート。
  for (const k of VISIT_LIST_KEYS) {
    if (Array.isArray(r[k])) return r[k].filter(Boolean).length >= 2;
  }
  return null;                                            // 🔴 分からないものは 0 にしない
}

// 初回に来店したか（来ていない人はリピートの母数に入れない）
export function visitedOf(row) {
  const r = (row && typeof row === 'object') ? row : null;
  if (!r) return null;
  if (typeof r.visited_completed === 'boolean') return r.visited_completed;
  if (typeof r.visited_by_time === 'boolean') return r.visited_by_time;
  if ('first_appointment_status' in r) return str(r.first_appointment_status) === 'completed';
  return null;
}

// 新規客台帳の行を、担当（セラピスト）ごとに畳む。
// 戻り値: { [staffId or 名前]: { staffId, staffName, visited, repeat } }
// 🔴 1人でも「分からない」が混じれば、その担当のリピート数は null にする（少なく見せない）。
export function repeatByStaff(rows) {
  const list = arr(rows);
  if (!list.length) return null;
  const out = new Map();
  for (const r of list) {
    if (!r || typeof r !== 'object') continue;
    const id = str(r.staff_id ?? r.staffId ?? '').trim();
    const name = str(r.staff_name ?? r.staffName ?? r.staff ?? '').trim();
    const key = id || name;
    if (!key) continue;
    const cur = out.get(key) || { staffId: id, staffName: name, visited: 0, repeat: 0, unknown: false };
    if (name && !cur.staffName) cur.staffName = name;
    const v = visitedOf(r);
    if (v === false) { out.set(key, cur); continue; }     // 来ていない人は母数に入れない
    if (v === null) { cur.unknown = true; out.set(key, cur); continue; }
    cur.visited += 1;
    const rep = repeatStateOf(r);
    if (rep === null) cur.unknown = true;
    else if (rep === true) cur.repeat += 1;
    out.set(key, cur);
  }
  if (!out.size) return null;
  const res = {};
  for (const [k, v] of out) {
    res[k] = { staffId: v.staffId, staffName: v.staffName,
               visited: v.unknown ? null : v.visited,
               repeat: v.unknown ? null : v.repeat };
  }
  return res;
}

// セラピスト別の行へリピート数を差し込む。担当は staff_id を優先し、無ければ名前で合わせる。
export function mergeRepeat(staff, byStaff) {
  const list = arr(staff);
  if (!list.length) return list;
  const m = (byStaff && typeof byStaff === 'object' && !Array.isArray(byStaff)) ? byStaff : null;
  if (!m) return list;
  const byName = {};
  for (const v of Object.values(m)) if (v && v.staffName) byName[v.staffName] = v;
  return list.map((x) => {
    const hit = m[str(x.id)] || byName[str(x.name)] || null;
    if (!hit) return x;
    return { ...x, repeat: isNum(hit.repeat) ? hit.repeat : null };
  });
}
