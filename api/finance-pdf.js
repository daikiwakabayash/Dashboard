...
## 抽出ルール
- 金額は全て円（整数）で出力
  - 千円単位の場合は×1000に変換（例: 25,600千円 → 25600000）
  - 百万円単位の場合は×1000000に変換
  - 万円単位の場合は×10000に変換
- マイナスの数値は負の整数で表現（△はマイナスを意味する）
- 該当データがない項目は 0 にする
- 複数期がある場合は最新の期のデータを抽出
- 科目名が異なる場合は最も近い科目にマッピング
- 画像が不鮮明でも、見える範囲で最善の読み取りをする
- **重要**: 売上高(sales)は「売上高」「営業収益」「収入合計」等の科目から必ず読み取ること
- 売上原価が明示されていない場合はgrossProfit=salesとし、costOfSales=0とする
- period は決算期末の年月を "YYYY-MM" 形式で出力（例: 令和5年10月期 → "2023-10"）
- fiscalYear は「第N期」の形式で出力
