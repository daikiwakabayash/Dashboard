...
describe('chat-rooms: 既存 Room の ID 紐付け', () => {
  it('既存 Room の ID は変えず storeId を後付けする', () => {
    ...
    expect(plan.bind).toEqual([
      { roomId: 'store_NAORU渋谷院', storeId: '100', shopName: 'NAORU渋谷院', matchedBy: 'exact_name' },
      { roomId: 'store_NAORU梅田院', storeId: '200', shopName: 'NAORU梅田院', matchedBy: 'exact_name' },
    ]);
...
describe('chat-rooms: 店舗名変更', () => {
  it('店舗名が変わっても同じ Room を維持し、表示名だけ更新する', () => {
    ...
    const plan = planStoreRoomSync({ rooms, shops, staffs: [{ id: 's1', name: 'A', shop_id: '100' }] });
    expect(plan.rename).toEqual([{ roomId: 'store_NAORU渋谷院', from: 'NAORU渋谷院', to: 'NAORU渋谷道玄坂院', storeId: '100' }]);
...
describe('chat-rooms: 所属メンバー（複数店舗・兼務・異動・退職）', () => {
  ...
  it('異動: 旧店舗から外し、新店舗へ入れる（自動所属のみ）', () => {
    ...
    const plan = planStoreRoomSync({ rooms, shops: SHOPS, staffs });
    expect(plan.memberRemove).toEqual([{ roomId: 'r100', storeId: '100', staffIds: ['s1'], reason: expect.any(String) }]);
...
describe('chat-rooms: 取得失敗・不完全な名簿', () => {
  ...
  it('スタッフ名簿が空でも「全員退職扱い」にしない（追加のみ・削除は保留）', () => {
    ...
    expect(plan.memberRemove).toEqual([]);
    expect(plan.review.some(r => r.kind === 'member_remove_held')).toBe(true);
  });
  it('スタッフ名簿が不完全なら削除を保留する', () => {
    const rooms = [{ id: 'r100', kind: 'store', name: 'NAORU渋谷院', shop: 'NAORU渋谷院', storeId: '100', members: ['s1'], autoMembers: ['s1'] }];
    const plan = planStoreRoomSync({ rooms, shops: [SHOPS[0]], staffs: [{ id: 's9', name: 'X', shop_id: '100' }], source: { staffsComplete: false } });
    expect(plan.memberRemove).toEqual([]);
    expect(plan.review.some(r => r.kind === 'member_remove_held')).toBe(true);
  });
  ...
  it('通常の異動・退職（少人数の削除）は上限に引っかからない', () => {
    ...
    const plan = planStoreRoomSync({ rooms, shops: [SHOPS[0]], staffs });
    expect(plan.memberRemove[0].staffIds.length).toBe(3);
  ...
  it('checkSourceHealth 単体: 正常なら削除を許可', () => {
    const h = checkSourceHealth({ shops: SHOPS, staffs: STAFFS });
  ...
  it('SalonOne に無い店舗の Room は削除せず archive 候補にする', () => {
    ...
    const plan = planStoreRoomSync({ rooms, shops: SHOPS, staffs: STAFFS });
    expect(plan.archive).toEqual([{ roomId: 'r900', reason: expect.stringMatching(/store_id=900 が存在しない/) }]);
  });
});
...
describe('chat-rooms: イベント Room（既存の evSaveChat / evJoinChat を前提）', () => {
  ...
  it('責任者と参加者を追加し、eventId を後付けする（kind は group のまま）', () => {
    ...
    expect(plan.bind).toEqual([{ roomId: 'g1', eventId: 'e1', matchedBy: 'event_row', name: '9月勉強会' }]);
  ...
  it('名簿に無い参加者IDは追加しない', () => {
    ...
    const plan = planEventRoomSync({ rooms, events: ev, staffs: STAFFS });
    expect(plan.memberAdd[0].staffIds).toEqual(['s1']);
    expect(plan.review.some(r => r.kind === 'event_member' && r.staffIds.includes('ghost'))).toBe(true);
  });
